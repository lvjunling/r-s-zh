...
StartMainScript = function()
    ...
    local function ExecuteAutoSell()
        ...
        pcall(function()
            local pg = player:FindFirstChild("PlayerGui")
            local flock = pg and pg:FindFirstChild("Collection") and pg.Collection:FindFirstChild("鸡群", true)
            if not flock then
                isSellingNow = false
                return
            end
    ...
    Title.Text = "ERDEVA HUB v2.7 中文免秘钥版"
    ...
    AddRarityChip(r3, "神话及以上", "SellMythic", Color3.fromRGB(231, 76, 60), true)
    ...
    AddButton(PlotPage, "[锁定] 设置回收机位置", function(btn)
        local root = GetRoot()
        if root then
            LOCKED_RECYCLER_POS = root.Position
            btn.Text = "回收机位置已锁定"
            Notify("ERDEVA HUB", "回收机位置已锁定", 3.0)
    ...
    AddInfo("已携带废料", "0 / 20", true)
    AddInfo("版本状态", "中文免秘钥版", false)
