...
KozakHub.Tabs.Home = KozakHub.Window:AddTab({ Title = "首页", Icon = "home" })
    ...
    if lotsFolder then
        for _, lot in ipairs(lotsFolder:GetChildren()) do
            ...
            if ownerName then
                ...
                local label = string.format("%s 的房屋（%s）", ownerName, lotNum)
...
KozakHub.Tabs.Player = KozakHub.Window:AddTab({ Title = "玩家", Icon = "user" })
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") then
                ...
                if tag and tag:FindFirstChild("TagLabel") then
                    ...
                    tag.TagLabel.Text = p.Name .. " [" .. dist .. "米]"
                end
            end
        end
    end
...
KozakHub.Tabs.Settings = KozakHub.Window:AddTab({ Title = "设置", Icon = "settings" })
...
KozakHub.Tabs.Troll = KozakHub.Window:AddTab({ Title = "整蛊", Icon = "sword" })
...
local function getPlayerNames()
    ...
    return #names > 0 and names or {"暂无玩家"}
end

if KozakHub.Tabs and KozakHub.Tabs.Troll then
    local initialNames = getPlayerNames()

    if initialNames[1] and initialNames[1] ~= "暂无玩家" then
    ...
    local PlayerDropdown = KozakHub.Tabs.Troll:AddDropdown("SelectTargetDropdown", {
        Title = "选择目标",
        Description = "自动更新玩家列表",
        Values = initialNames,
        Multi = false,
        Default = 1,
        Callback = function(Value)
            if Value and Value ~= "暂无玩家" then
        ...
        task.defer(function()
            pcall(function()
                ...
                if KozakHub.TargetPlayer then
                    ...
                    if currentNames[1] and currentNames[1] ~= "暂无玩家" then
...
if KozakHub.Tabs and KozakHub.Tabs.Troll then
    KozakHub.Tabs.Troll:AddToggle("WatchTargetToggle", {
        Title = "观察目标",
        Description = "将镜头锁定到所选玩家（重生后继续生效）",
        Default = false,
        Callback = function(Value)
...
KozakHub.Tabs.Vehicle = KozakHub.Window:AddTab({ Title = "载具", Icon = "car" })
...
if KozakHub.Tabs and KozakHub.Tabs.Vehicle then
    KozakHub.Tabs.Vehicle:AddToggle("WheelSpinToggle", {
        Title = "载具车轮旋转",
        Description = "让载具车轮以最高速度旋转（期间请勿离开载具！）",
        Default = false,
        Callback = function(Value)
